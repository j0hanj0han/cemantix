...
{FEED_LINK_TAG}
  ...
  {{
    ...
    "itemListElement": [
      ...
      {{"@type": "ListItem", "position": 4, "name": "{date_str[:4]}", "item": "{LOTO_SITE_URL}/archive/{date_str[:4]}"}},
...
def _year_row(e: dict) -> str:
    d = date.fromisoformat(e["date"])
    balls_str = " · ".join(str(b) for b in e["balls"])
    return (
        '        <tr>'
        f'<td class="arch-date">{date_fr(d)}</td>'
        f'<td class="arch-num">n°{e.get("draw_num", "")}</td>'
        f'<td><a class="arch-link" href="{e["date"]}">{balls_str} + {e["lucky_ball"]}</a></td>'
        '</tr>'
    )


def generate_year_html(year: str, entries: list[dict], prev_year, next_year) -> None:
    """Génère docs/loto/archive/YYYY.html — récap de tous les tirages de l'année."""
    LOTO_ARCHIVE.mkdir(parents=True, exist_ok=True)
    count = len(entries)
    stats = compute_loto_stats(entries)
    top_balls_str = " · ".join(f"{b} ({c}×)" for b, c in stats["top_balls"][:5])

    rows_html = "\n".join(_year_row(e) for e in entries)
...
{FEED_LINK_TAG}
  ...
  <meta property="og:title" content="Loto {year} — Tous les tirages">
  <meta property="og:description" content="Les {count} tirages Loto de {year} avec numéros gagnants et statistiques.">
  <meta property="og:type" content="website">
  <meta property="og:url" content="{LOTO_SITE_URL}/archive/{year}">
  ...
  {{
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    "name": "Loto {year} — Tous les tirages",
    "description": "Liste complète des {count} tirages Loto de {year} avec numéros gagnants.",
    "url": "{LOTO_SITE_URL}/archive/{year}",
  ...
  {{
    ...
    "itemListElement": [
      ...
      {{"@type": "ListItem", "position": 3, "name": "Archives", "item": "{LOTO_SITE_URL}/archive/"}},
...
<header class="site-header">
  <h1>Loto {year} — Tous les tirages</h1>
...
<nav class="breadcrumb" aria-label="Fil d'Ariane">
  ...
  <article>
    <div class="card">
      <h2>Tous les tirages Loto de {year}</h2>
      <p>
        Retrouvez les <strong>{count} tirages Loto de {year}</strong> avec leurs numéros gagnants
        et numéro chance. Boules les plus fréquentes de l'année : <strong>{top_balls_str}</strong>.
      </p>
        ...
        <table class="month-table" style="width:100%;border-collapse:collapse;">
          <thead>
            <tr>
              ...
              <th style="text-align:left;">Tirage</th>
...
<footer>
  ...
  <p style="margin-top:.4rem;">Site non officiel — Résultats récupérés automatiquement</p>
    ...
    atomic_write(LOTO_ARCHIVE / f"{year}.html", html)


def generate_archive_index(entries: list[dict], years: dict[str, list] | None = None, recent_limit: int = 50) -> None:
    """Génère docs/loto/archive/index.html — les `recent_limit` tirages les plus récents + une card "Par année"."""
    LOTO_ARCHIVE.mkdir(parents=True, exist_ok=True)
    ...
    def item_html(e: dict) -> str:
        ...
        return (
            ...
            f'<a class="arch-link" href="{e["date"]}">{balls_str} + {e["lucky_ball"]}</a>'
            f'</li>'
        )
...
<nav class="breadcrumb" aria-label="Fil d'Ariane">
  ...
  <div class="card">
    <h2>Les {len(recent)} derniers tirages Loto (sur {count} au total)</h2>
    <p style="font-size:.9rem;color:#6b7280;margin-bottom:1rem;">
    ...
    if recent_archives:
        def arch_item(e: dict) -> str:
            ...
            return (
                ...
                f'<span class="arch-date">{date_fr(d)}</span>'
                f'<span class="arch-num">n°{e.get("draw_num", "")}</span>'
                f'<a class="arch-link" href="archive/{e["date"]}">{b_str} + {e["lucky_ball"]}</a>'
                f'</li>'
            )
...
{retard_rows}
  ...
  <div style="text-align:center;margin-top:.5rem;">
    <a class="reveal-btn" href="../">Dernier tirage Loto &#8594;</a>
  </div>
</main>
...
def _generate_all_html(draw_date: date, data: dict) -> None:
    ...
    print(f"[Loto] Génération des pages annuelles ({len(year_keys)} années)…")
